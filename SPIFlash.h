...
#define SPIFLASH_ARRAYREAD        0x0C        // read array (fast, need to add 1 dummy byte after 3 address bytes)
#define SPIFLASH_ARRAYREADLOWFREQ 0x13        // read array (low frequency)

#define SPIFLASH_SLEEP            0xB9        // deep power down
#define SPIFLASH_WAKE             0xAB        // deep power wake up
#define SPIFLASH_BYTEPAGEPROGRAM  0x12        // write (1 to 256bytes)
...
public:
  ...
  boolean initialize();
  boolean command(uint8_t cmd, boolean isWrite=false);
  uint8_t readStatus();
  boolean readByte(uint32_t addr, uint8_t* result);
  boolean readBytes(uint32_t addr, void* buf, uint16_t len);
  boolean writeByte(uint32_t addr, uint8_t byt);
  uint16_t writeBytes(uint32_t addr, const void* buf, uint16_t len);
  boolean busy();
  boolean chipErase();
  boolean blockErase4K(uint32_t address);
  boolean blockErase32K(uint32_t address);
